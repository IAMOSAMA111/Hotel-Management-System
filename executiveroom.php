...
<body background="hotel2.jpg"  id="b1">
  ...
  <tr>
		<td class="label">Receptionist cnic</td>
		<td><input  id ="idbox" type="text" name="recep_cnic" value="" required><br><br></td>
	</tr>
	</table>
...
<?php
 ...
 $rcnic=	$_GET['recep_cnic'];
 $ravail= $_GET['roomavailible'];


 $query1="INSERT INTO mydb.user
  				VALUES ('','$mname','$mph','$memail','$maddress','$mcountry','$mdob')";
 $data = mysqli_query($conn,$query1);
 if($data==false){
 	echo mysqli_error($conn);
 }
 $getdata = mysqli_insert_id($conn);
 if($getdata==false){
 	echo mysqli_error($conn);
 }
 $data3=mysqli_query($conn,$queryrecep);
 $queryupdate="UPDATE room set roomAvailibility='No' where roomID='$ravail' ";
 $data4=mysqli_query($conn,$queryupdate);
 $query2 = "INSERT INTO mydb.guest VALUES ('','$mcnic','$getdata','$ravail',null,
 'SELECT receptionistID from mydb.receptionist where
 receptionistCNIC ='$rcnic','$gstatus')";
